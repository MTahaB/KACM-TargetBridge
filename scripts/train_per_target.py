...
def main():
    ...
    ap.add_argument(
        "--csv", default=None, help="Optional CSV with smiles and endpoint column"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="Seuil OOD composite (sinon 90e percentile train)",
    )
    ap.add_argument(
        "--gamma",
        type=float,
        default=1.5,
        help="Amplification adaptative des intervalles",
    )
    ap.add_argument(
        "--z_gp", type=float, default=1.0, help="poids σ_GP dans l'intervalle hybride"
    )
    args = ap.parse_args()
    ...
    ac = AdaptiveConformalRegressor(
        model=base, alpha=0.1, gamma=args.gamma, k_dens=8
    ...
    tau = (
        args.ood_tau if args.ood_tau is not None else float(np.quantile(oods_tr, 0.90))
    )
    ...
    print(
        f"[{args.chembl_id}] PICP={picp:.3f} | MPIW={mpiw:.3f} | "
        f"sigma_GP~{sig_gp.mean():.3f} | OOD_tau~{tau:.2f} | "
        f"artifacts -> {outdir}"
    )
