...
def main():
    ...
    ap.add_argument(
        "--alpha", type=float, default=0.1, help="Miscoverage level (1-coverage)"
    )
    args = ap.parse_args()
    ...
    print(
        f"[{args.chembl_id}] CQR-HGBR  PICP={picp:.3f} | "
        f"MPIW={mpiw:.3f} | qhat={cqr.qhat_:.3f}"
    )
