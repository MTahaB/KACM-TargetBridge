...
class BayesianOptimizer:
    ...
    def _gaussian_process_surrogate(
        self, X_candidate: np.ndarray
    ) -> Tuple[float, float]:
        ...
        var = np.var(self.y_obs) * (
            1.0 - weights.max()
        )  # Incertitude réduite près des observations
...
def optimize_conformal_hyperparams(
    X_train: np.ndarray,
    y_train: np.ndarray,
    X_val: np.ndarray,
    y_val: np.ndarray,
    ...
    for i, result_i in enumerate(all_results):
        ...
        for j, result_j in enumerate(all_results):
            if i != j:
                ...
                if (
                    result_j["objectives"][0] >= result_i["objectives"][0]
                    and result_j["objectives"][1] >= result_i["objectives"][1]
                    and (
                        result_j["objectives"][0] > result_i["objectives"][0]
                        or result_j["objectives"][1] > result_i["objectives"][1]
                    )
                ):
                    is_dominated = True
                    break
