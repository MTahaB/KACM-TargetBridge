...
def quantile_calibration(residuals: np.ndarray, alpha: float) -> float:
    n = residuals.shape[0]
    if n == 0:
        raise ValueError("Cannot calibrate conformal intervals with no residuals.")
...
class AdaptiveConformalRegressor:
    ...
    def fit_calibrate(self, X_train, y_train, X_cal, y_cal):
        ...
        s_cal = 1.0 + self.gamma * (
            1.0 - np.array([density_score(x, X_train, k=self.k_dens) for x in X_cal])
        )
        scaled = resid / s_cal
    ...
    def predict_interval(self, X):
        ...
        s = 1.0 + self.gamma * (
            1.0 - np.array([density_score(x, self.X_train_, k=self.k_dens) for x in X])
        )
        w = self.qhat_ * s
        lo, hi = mu - w, mu + w
        return mu, lo, hi
