...
try:
    ...
    HAVE_3D = True
    _3D_IMPORT_ERROR = None
except ImportError as exc:
    HAVE_3D = False
    _3D_IMPORT_ERROR = exc
...
st.write(
    "🚀 **Advanced ML Platform** with **Gaussian Process variance**, **Conformalized Quantile Regression**, and **hybrid uncertainty quantification**."
)

# 3D capabilities info
if HAVE_3D:
    st.success(
        "🌐 **Visualisation 3D activée** - Modèles moléculaires interactifs disponibles"
    )
else:
    st.info(
        f"💡 **Pour activer la 3D**: `pip install py3Dmol` puis redémarrez l'application ({_3D_IMPORT_ERROR})"
    )

# Sidebar
st.sidebar.header("⚙️ Settings")
mode = st.sidebar.selectbox(
    "Ranking mode", ["Prudent (μ)", "Explorateur (UCB-GP)", "Explorateur (UCB-width)"]
)
beta = st.sidebar.slider("UCB β (exploration)", 0.0, 2.0, 0.6, 0.1)
abstain_w = st.sidebar.slider(
    "Seuil largeur d'intervalle pour abstention", 0.5, 3.0, 1.2, 0.1
)

# Model selection
model_type = st.sidebar.selectbox(
    "Model Type", ["KRR+Conformal", "CQR-HGBR", "Ensemble Average"]
)

# Visualization options
st.sidebar.header("🎨 Visualisation")
show_3d = st.sidebar.checkbox("Visualisation 3D", value=HAVE_3D, disabled=not HAVE_3D)
show_molecular_gallery = st.sidebar.checkbox(
    "Galerie moléculaire 3D", value=False, disabled=not HAVE_3D
)

art_dir = Path("artifacts")
targets = (
    sorted([p.name for p in art_dir.iterdir() if p.is_dir()])
    if art_dir.exists()
    else []
)
...
def show_3d_viewer(viewer, height=400, width=400):
    """Render a py3Dmol viewer in Streamlit without the deprecated stmol wrapper."""
    st.iframe(viewer._make_html(), height=height, width=width)
...
def enhanced_molecular_depiction(smiles, title="Molecule"):
    ...
    with col2:
        ...
        if HAVE_3D:
            ...
            if viewer_3d:
                show_3d_viewer(viewer_3d, height=400, width=400)
            else:
                st.warning("Impossible de générer le modèle 3D")
        else:
            st.info("📦 Installez py3Dmol pour la visualisation 3D")
...
def predict_ensemble(smi, packs, mode, beta):
    ...
    score = (
        mu_avg + beta * (hi_avg - mu_avg) if not mode.startswith("Prudent") else mu_avg
    )
...
if btn:
    if not use_targets:
        st.warning(
            "Aucun modèle trouvé dans artifacts/. Entraîne d'abord quelques cibles."
        )
    else:
        df_in = None
        if up is not None:
        ...
        if df_in is None or df_in.empty:
            ...
            if not rows:
                ...
                res = res.sort_values(
                    by=["smiles", "score"], ascending=[True, False]
                ...
                with col1:
                    ...
                    st.markdown(
                        f"**Cible**: `{s0['target']}`  |  **Méthode**: `{s0['method']}`"
                    )
                    st.markdown(
                        f"**μ** ≈ {s0['mu']:.2f}  |  **Intervalle**: [{s0['lo']:.2f}, {s0['hi']:.2f}]"
                    )

                    if "sigma_gp" in s0:
                        st.markdown(
                            f"**σ_GP** ≈ {s0['sigma_gp']:.3f} | **OOD** ≈ {s0['ood']:.2f}"
                        )
                    else:
                        st.markdown(f"**OOD** ≈ {s0['ood']:.2f}")

                    decision_text = (
                        "✅ *Prédire*" if not s0["abstain"] else "🟡 *S'abstenir*"
                    )
                    ...
                    if s0["method"] == "CQR":
                        ...
                        st.info(
                            f"🤖 **Ensemble**: Moyenne de {s0.get('n_models', 'N')} modèles"
                        )
                ...
                with col2:
                    ...
                    original_pred = predict_one(
                        s0["smiles"],
                        {s0["target"]: packs[s0["target"]]},
                        mode,
                        beta,
                        model_type,
                    )
                    if original_pred and "knn" in original_pred:
                        for i, v in enumerate(original_pred["knn"]):
                            st.markdown(
                                f"**Voisin {i+1}** - Sim: {v['sim']:.2f} | pIC50: {v['pIC50']:.2f}"
                            )

                            # Visual representation of neighbor
                            if HAVE_3D and show_3d:
                                viewer_neighbor = depict_3d(
                                    v["smiles"], height=200, width=250
                                )
                                if viewer_neighbor:
                                    show_3d_viewer(
                                        viewer_neighbor, height=200, width=250
                                    )
                                else:
                            ...
                            st.code(
                                f"{v['smiles'][:50]}{'...' if len(v['smiles']) > 50 else ''}",
                                language=None,
                            )
                ...
                with col3:
                    ...
                    if HAVE_3D and len(neighbor_smiles) > 0:
                        molecules_to_compare = [s0["smiles"]] + neighbor_smiles[
                            :2
                        ]  # Input + 2 neighbors
                        labels = ["Entrée"] + [
                            f"Voisin {i+1}" for i in range(len(neighbor_smiles[:2]))
                        ]

                        viewer_comparison = compare_molecules_3d(
                            molecules_to_compare, labels
                        )
                        if viewer_comparison:
                            show_3d_viewer(viewer_comparison, height=350, width=300)
                        else:
                ...
                if show_molecular_gallery and HAVE_3D and len(res) > 1:
                    ...
                    st.write(
                        "Autres molécules du dataset avec leurs scores de prédiction"
                    )

                    # Get unique molecules excluding the input molecule
                    other_molecules = (
                        res[res["smiles"] != s0["smiles"]]
                    ...
                    if len(other_molecules) > 0:
                        ...
                        molecule_tabs = st.tabs(
                            [
                                f"Mol {i+1} (Score: {row['score']:.1f})"
                                for i, (_, row) in enumerate(other_molecules.iterrows())
                            ]
                        )

                        for tab, (_, mol_data) in zip(
                            molecule_tabs, other_molecules.iterrows()
                        ):
                            with tab:
                                ...
                                with col1:
                                    if show_3d:
                                        viewer_3d = depict_3d(
                                            mol_data["smiles"], height=300, width=350
                                        )
                                        if viewer_3d:
                                            show_3d_viewer(
                                                viewer_3d, height=300, width=350
                                            )
                                        else:
